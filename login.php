...
// [Fix] Defensive POST initialization
$getData = $_POST ?? [];
$getSettings = new Settings($config_setting_file_path);
$cpsub = $getSettings->getSettings();

// [Fix] Ensure $cpsub keys exist
$filter_val = $cpsub['filter'] ?? '0';
$stripslashes_val = $cpsub['stripslashes'] ?? '0';

$getLib = new Lib($filter_val, $stripslashes_val);
...
// get title
// [Fix] Defensive array access
$website_title = $getLib->setFilter($cpsub['title'] ?? '') . "-" . $website_current_page;

// set add function
// [Fix] Only attempt login if send is pressed to avoid unnecessary processing on GET
$getResult = ['status' => false, 'msg' => []];
if (isset($getData['send'])) {
	$getResult = $getAuth->setLogin($getData);
}

// [Fix] Session check with defensive coding
			...
			<div class="panel-body">
				...
				<form class="form-horizontal" role="form" action="login.php" method="post">
					<div class="form-group">
						...
						<div class="col-lg-10">
							<input type="text" class="form-control" id="cpsub_username" name="cpsub_username"
								placeholder="請輸入帳號">
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-lg-10">
							<input type="password" class="form-control" id="cpsub_password" name="cpsub_password"
								placeholder="請輸入密碼">
						</div>
					</div>
					<div class="form-group">
