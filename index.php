		...
		<div class="navbar">
			...
			<form class="navbar-form navbar-right" role="search" action="index.php" method="get">
				<div class="form-group">
					<input type="text" class="form-control" placeholder="輸入關鍵字" value="<?= $getKeyword; ?>"
						name="keyword">
				</div>
		...
		<div class="panel panel-default">
			<table class="table table-hover">
				...
				<tbody>
					<?php
					$count = 0;
					// [Fix] Use alternative syntax for better readability
					foreach ($getListArray as $getKey => $getVal):
						if ($count >= $pageStart && $count < ($pageMany + $pageStart)) {
							...
							<tr>
								<td><?= $article_id; ?></td>
								<td><?= $article_top; ?><a href="article.php?id=<?= $article_id; ?>"><?= $article_title; ?></a></td>
